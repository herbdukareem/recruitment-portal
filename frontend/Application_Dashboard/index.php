		...
		<script>
			...
			const checkSession = async () => {
				try {
					...
					const response = await fetch(`${API_URI}auth/user/session`, {
						method: 'GET',
						credentials: 'include',
						headers: {
		...
		const submitForm = async (endpoint, formData) => {
			...
			const fetchUserProfile = async () => {
				try {
					...
					const response = await fetch(`${API_URI}user/data?user_id=${user.id}`, {
						method: 'GET',
						headers: {
							'Content-Type': 'application/json',
			...
			const fetchUserBio = async (user_cred = {}) => {
				try {
					...
					const response = await fetch(`${API_URI}user/bio?user_id=${user.id}`, {
						method: 'GET',
						headers: {
							'Content-Type': 'application/json',
							'Authorization': `Bearer ${token}`
			...
			function populateUserData(user_data, user_cred) {
				...
				document.querySelector('input[name="firstname"]').value = user_data.firstname ? user_data.firstname : (user_cred.firstname || '');
				document.querySelector('input[name="middlename"]').value = user_data.middlename || '';
				document.querySelector('input[name="lastname"]').value = user_data.lastname ? user_data.lastname : (user_cred.lastname || '');
				if (user_data.email) {
